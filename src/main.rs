...
fn ameotrack_upload<P: AsRef<Path>>(
    ...
    let res_text = res.text().expect("Unable to parse HTTP response into text!");
...
pub fn main() {
    ...
    loop {
        ...
        texture.update(None, &*frame, width * 4)
        ...
        | -> Result<(), Box<Error>> {
            ...
            let mut ctx: ClipboardContext = ClipboardProvider::new()
                .expect("Unable to create clipboard context!");
            ctx.set_contents(image_url.clone()).expect("Unable to set clipboard contents!");
